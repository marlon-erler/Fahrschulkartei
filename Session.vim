...
silent only
silent tabonly
cd ~/Documentos/Code/Commercial/Fahrschulkartei
...
badd +10 Backend/Core/server.ts
badd +0 Backend/UI/Pages/student.ts
argglobal
%argdel
...
exe 'vert 3resize ' . ((&columns * 80 + 96) / 192)
argglobal
enew
...
wincmd w
argglobal
if bufexists(fnamemodify("Backend/UI/Pages/student.ts", ":p")) | buffer Backend/UI/Pages/student.ts | else | edit Backend/UI/Pages/student.ts | endif
balt Backend/Core/server.ts
let s:cpo_save=&cpo
set cpo&vim
nmap <buffer> [c <Plug>(GitGutterPrevHunk)
